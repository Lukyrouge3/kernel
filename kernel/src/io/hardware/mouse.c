...
void mouse_handler(void) {
    ...
    serial_printf("  Byte[%d]: 0x%X\n", mouse_cycle, mouse_data);

    switch (mouse_cycle) {
    case 0:
        mouse_packet[0] = mouse_data;
        if (!(mouse_data & 0x08)) {
            serial_printf("  -> Invalid byte 0 (bit3 not set), waiting for sync\n");
            break; // Invalid, stay at 0
        }
        mouse_cycle++;
        break;
    ...
    case 2:
        ...
        serial_printf("  PACKET: Buttons=0x%X, X=%d, Y=%d\n", mouse_packet[0] & 0x07, delta_x,
                      delta_y);
        break;
    }
}
...
void mouse_init(void) {
    ...
    ps2_write_cmd(WRITE_TO_MOUSE);
    ps2_write_data(200); // Set sample rate
    response = ps2_read_data();
    ...
    // Small delay to let mouse stabilize
